...
async def health_check():
    ...
    health_status = {
        ...
        "timestamp": "2024-01-01T00:00:00Z"  # You could add datetime.utcnow().isoformat()
    }
    ...
    if data_source == "mqtt":
        ...
        if hasattr(weather_service, '_client'):
            mqtt_status = "connected" if weather_service._client.is_connected() else "disconnected"
            health_status["mqtt_status"] = mqtt_status
            health_status["mqtt_broker"] = f"{weather_service.broker_host}:{weather_service.broker_port}"
